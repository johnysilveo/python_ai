...
class Product(Money):
    ...
    def reduce_price(self, whole_part, fractional_part=0):
        reduction = whole_part * 100 + fractional_part
        current_price = self.get_total_cents()
        if reduction < 0:
            print("Reduction cannot be negative.")
            return
        if reduction > current_price:
            print("Reduction cannot be greater than the price.")
            return
        new_price = current_price - reduction
        self.set_amount(0, new_price)
